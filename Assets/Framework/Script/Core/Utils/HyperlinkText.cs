...
public class HyperlinkText : Text, IPointerClickHandler
{
    ...
    /// 超链接正则
    /// </summary>
    private static readonly Regex s_HrefRegex =new Regex(pattern, RegexOptions.Singleline);
        ...
        if( Application.isPlaying )
        {
            ...
            foreach( var hrefInfo in hrefInfos )
            {
                foreach( var area in hrefInfo.textValidAreas )
                {
                    ...
                    RectToDrawInfo rectToDrawInfo =new RectToDrawInfo(screenPosMin, screenPosMax);
    ...
    protected virtual string GetDealedText(string originText)
    {
        ...
        foreach( Match match in s_HrefRegex.Matches(originText) )
        {
            ...
            beforeStr = originText.Substring(matchNextCharIndex, match.Index - matchNextCharIndex);
    ...
    private static void DebugFunc(int actualStartIndex, string beforeStr, string beforeStrNoBlank, Match match, Group group)
    {
        ...
        {
            str += $" match.Groups[{i}] " +match.Groups[i] + " match.Length " + match.Length + "\n" ;
        }

        print(str +
            " HyperlinkText GetOutputText  firstStr " + beforeStr +
            "   " + match.Groups[2] +
