...
namespace FrameWork
{
    public class CreateFont : EditorWindow
    {
        ...
        private void Create()
        {
            if (tex == null)
            {
                DebugUtil.LogWarn("创建失败，图片为空！", "");
                return;
            }

            if (string.IsNullOrEmpty(fontPath))
            {
                DebugUtil.LogWarn("字体路径为空！", "");
                return;
            }
            if (fontName == null)
            {
                DebugUtil.LogWarn("创建失败，字体名称为空！", "");
                return;
            }
            else
            {
                ...
                {
                    DebugUtil.LogError("创建失败，已存在同名字体文件","");
                    return;
                }
                if (File.Exists(Application.dataPath + fontPath + fontName + ".mat"))
                {
                    DebugUtil.LogError("创建失败，已存在同名字体材质文件","");
                    return;
                }
            }
            ...
            if (selectionPath.Contains("/Resources/"))
            {
                ...
                if (sprites.Length > 0)
                {
                    ...
                    for (int i = 0; i < sprites.Length; i++)
                    {
                        ...
                        {
                            DebugUtil.LogError("创建失败，Sprite名称错误！","");
                            return;
                        }
                        Rect rect = spr.rect;
                    ...
                    DebugUtil.Log("创建字体成功","");

                }
                else
                {
                    DebugUtil.LogError("图集错误！","");
                }
            }
            else
            {
                DebugUtil.LogError("创建失败,选择的图片不在Resources文件夹内！","");
            }
        }
    }
}