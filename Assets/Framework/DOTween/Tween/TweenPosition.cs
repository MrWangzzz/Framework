...
public class TweenPosition : UITweener
{
    ...
    private void Once (Vector3 from, Vector3 to)
    {
        ...
        CacheTransform. DOLocalMove(to, duration).SetEase(ease). OnComplete(() => onFinished());
    }
    ...
    {
        CacheTransform. localPosition = from;
        CacheTransform. DOLocalMove(to, duration).SetEase(ease). OnComplete(() => Loop(this. from, to));
    }
    ...
    {
        CacheTransform. localPosition = from;
        CacheTransform. DOLocalMove(to, duration).SetEase(ease). OnComplete(() => CacheTransform. DOLocalMove(this. from, duration).SetEase(ease));
    }
    ...
    {
        CacheTransform. DOLocalMove(to, duration).SetEase(ease). OnComplete(() => PingPong(to, from));
    }
